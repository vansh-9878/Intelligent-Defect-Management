import { LayoutDashboard, FileText, ListChecks, BarChart3, User } from 'lucide-react';
import { motion } from 'motion/react';

type Screen = 'dashboard' | 'report-defect' | 'defect-list' | 'analytics';
...
export function TopNav({ onNavigate, currentScreen }: TopNavProps) {
  const navItems = [
  ...
  return (
    <nav className="bg-[#1A1A1A] border-b border-gray-800 h-16 px-6 flex items-center justify-between">
      <div className="flex items-center gap-8">
        <h1 className="text-xl text-gray-100">Intelligent Defect Management Platform</h1>
            ...
            return (
              <motion.button
                ...
                className={`px-4 py-2 rounded-lg flex items-center gap-2 transition-colors relative ${
                  isActive ? 'text-[#3B9EBF]' : 'text-gray-400 hover:text-gray-200'
                }`}
